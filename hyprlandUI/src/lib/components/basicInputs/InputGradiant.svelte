...
<div class="flex w-full flex-col gap-3" bind:this={divEl}>
	<div class="flex flex-row items-center justify-between">
		...
		<div class="flex flex-row gap-3 items-center">
			...
			<button class="btn btn-sm btn-circle btn-soft btn-success">
	...
	<div class="dark flex w-full flex-col items-center justify-center gap-2">
		...
		<div class="join flex w-full flex-row justify-center">
			<button
				...
				onclick={() => (gradiant = [])}>Reset Gradiant</button
			>
			<button
				class="btn btn-info btn-soft join-item grow text-xs"
				onclick={() => gradiant.push(`rgba(${rgb.r} ,${rgb.g} ,${rgb.b} ,${rgb.a})`)}
				>Add To Gradiant</button
			>
		</div>
		<div class="flex w-full flex-col gap-2">
			{#each gradiant as color}
				<div
				...
				>
					...
					<div>
						...
						<button
							...
							onclick={() => (gradiant = [...gradiant.filter((item) => item !== color)])}
						>
							<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
								><path
									fill="currentColor"
