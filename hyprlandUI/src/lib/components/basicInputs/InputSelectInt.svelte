<script lang="ts">
	...
	let selectedItme = $state(ui.value as number | null);
	let focusOn = $state(null as string | null);

	let selectShow = $state(false);

	if (ui.optionExplain !== null && ui.valueOptions !== null && ui.value !== null) {
		selectedValue = ui.optionExplain[(ui.valueOptions as number[]).indexOf(ui.value as number)];
		focusOn = ui.optionExplain[(ui.valueOptions as number[]).indexOf(ui.value as number)];
	}
	...
	function changeSelectedValue(value: string) {
		...
		if (!isNaN(Number(value))) {
			const validNumber = (ui.valueOptions as number[])!!.includes(Number(value))

			if (validNumber && ui.optionExplain !== null) {
				focusOn = ui.optionExplain[(ui.valueOptions as number[])!!.indexOf(Number(value))]
			}
	...
	<div class="relative inline-block">
		<label class="input w-full bg-transparent">
			...
			<input
				...
				placeholder="Select The Correct Settings For Accel Profile"
				...
				onkeypress={(e) => {
					if (e.key.toLocaleLowerCase() === 'enter') {
						selectedItme = (ui.valueOptions as number[])[
							ui.optionExplain!!.indexOf(focusOn as string)
						];
						selectedValue = ui.optionExplain!![ui.optionExplain!!.indexOf(focusOn as string)];
		...
		<div
			class="bg-base-100 absolute z-20 mt-4 mb-8 flex h-fit w-full flex-col gap-3 border-1 p-4 {selectShow
				...
				: 'hidden'}"
		>
			{#if ui.optionExplain !== null}
				{#each ui.optionExplain as option, index}
					<div
						class="{selectedItme === ui.valueOptions!![index] ? 'bg-base-300' : ''}
				...
				flex flex-row items-center justify-between p-3"
					>
						...
						<div>
							...
							{#if selectedItme === ui.valueOptions!![index]}
